...
// Templates rotacionados a cada novo lead que cai no CRM ficam em
// bot/templates.js — fonte única compartilhada com disparo-massa.js.
...
// Stage 'novo' usa rotação aleatória local (TEMPLATES_BOAS_VINDAS) pra dar A/B
...
async function getStageTemplate(stage) {
  if (stage === 'novo' && TEMPLATES_BOAS_VINDAS.length > 0) {
    const i = Math.floor(Math.random() * TEMPLATES_BOAS_VINDAS.length);
    const escolhido = TEMPLATES_BOAS_VINDAS[i];
    console.log(`   🎲 Variante sorteada: ${i + 1}/${TEMPLATES_BOAS_VINDAS.length} (${escolhido.template_name})`);
    return escolhido;
  }
